...
export default function Directory({ html }: { html: string }) {
  ...
  return (
    <>
      ...
      <Drawer
        className={`${style['hide-on-widescreen']}`}
        position='right'
        size='75%'
        transitionProps={{
      ...
      >
        ...
        <div className='pl-3 text-slate-600'>{genNestNode(directory)}</div>
      </Drawer>
      <div className={`${style['hide-on-mobile']} mt-10`}>
        <div className='flex items-center mb-4 text-slate-600'>
          <Content />
          <p className='ml-4 text-2xl font-bold'>目录</p>
        </div>
        <div className='pl-3 text-slate-600 border-l'>{genNestNode(directory)}</div>
      </div>
    </>
  )
}
      ...
      {nodes.map((node) => {
        return (
          <List.Item key={`d${node.value}_${node.level}`}>
            <a
              className={`directory-${node.level} text-base text-slate-600 block mb-2`}
